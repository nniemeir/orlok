...
void handle_read_entry(pid_t pid, struct user_regs_struct *regs,
  ...
  // As such, casting to the actual type that was passed in is advised
  state->read_values.fd = (unsigned int)regs->rdi;
...
void handle_write_entry(pid_t pid, struct user_regs_struct *regs,
  ...
  state->write_values.buffer = malloc(ARG_MAX);
  if (!state->write_values.buffer) {
    fprintf(stderr, "Failed to allocate memory: %s\n", strerror(errno));
    exit(1);
  }
  read_string_arg(&state->write_values.buffer, pid, regs->rsi);
  state->write_values.count = (size_t)regs->rdx;
}
...
void handle_exit_entry(pid_t pid, struct user_regs_struct *regs,
                       syscalls_state *state) {
  (void)pid;
  state->exit_values.error_code = (int)regs->rdi;
}
...
void handle_stat_entry(pid_t pid, struct user_regs_struct *regs,
                       syscalls_state *state) {
  state->stat_values.filename = malloc(ARG_MAX);
  if (!state->stat_values.filename) {
    fprintf(stderr, "Failed to allocate memory: %s\n", strerror(errno));
    exit(1);
  }
  read_string_arg(&state->stat_values.filename, pid, regs->rdi);
}
...
void handle_accept_entry(pid_t pid, struct user_regs_struct *regs,
                         syscalls_state *state) {
  (void)pid;
  state->accept_values.sockfd = (int)regs->rdi;
  state->accept_values.addr = (void *)regs->rsi;
  state->accept_values.addrlen = (socklen_t)regs->rdx;
}
